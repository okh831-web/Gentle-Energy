...
const ExpertisePage: React.FC = () => {
  const sections = [
    ...
    {
      title: '핵심역량 분석',
      enTitle: 'Core Competency Analysis',
    ...
    <div className="py-20 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-sm font-bold text-emerald-700 uppercase tracking-widest mb-3">Expertise & Domains</h2>
          <h3 className="text-4xl font-bold text-slate-900 font-serif">전문 영역 <span className="text-slate-400 text-2xl font-medium">Areas of Excellence</span></h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {sections.map((sec, i) => (
            <div key={i} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-200">
              <div className={`px-8 py-6 border-b ${sec.color} font-bold text-xl flex flex-col`}>
                <span>{sec.title}</span>
                <span className="text-xs font-medium opacity-70 uppercase tracking-widest mt-1">{sec.enTitle}</span>
              </div>
              <div className="p-8">
                <div className="mb-8">
                  <h5 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Components (주요 항목)</h5>
                ...
                <div className="bg-slate-50 p-6 rounded-2xl">
                  <h5 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Deliverables (산출물 예시)</h5>
